define(
	...
	frame.run(function($rootScope,$state,$http,$q){
		...
		function getUserDetails(){
			$http({
				url:"http://yiyunma.com/api/v1/accounts/2",
				// url:"http://yiyunma.com/api/v1/accounts/" + getCookie("userNecessary") ,
				method:"get"
			}).then(function(data){
				// add header for valid
				// removeCookie("userNecessary")
				addHeader(data.data.data.account);
				$rootScope.users.customer = data.data.data.customer;
				$rootScope.users.owner_id = data.data.data.customer.id;
